...
class DiscreteGraphLearning(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.k = cfg["K"]       # the "k" of knn graph
        ...
        self.node_feats = torch.from_numpy(load_pkl("datasets/" + cfg.DATASET_NAME + "/data.pkl")).float()[:self.train_length, :, 0]
    ...
    def forward(self, long_term_history, tsformer):
        ...
        hidden_states = tsformer(long_term_history[..., [0]].permute(0, 2, 3, 1))
        dynamic_feat = F.relu(self.fc_mean(hidden_states.reshape(batch_size, num_nodes, -1)))     # relu(FC(Hi)) in Eq. (2)

        # time series feature
        node_feat = global_feat + dynamic_feat      # Zi in Eq. (2)
