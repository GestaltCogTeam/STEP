...
class DCGRUCell(torch.nn.Module):
    ...
    def forward(self, inputs, hx, sampled_adj):
        output_size = 2 * self._num_units
        if self._use_gc_for_ru:
            fn = self._gconv
        else:
            fn = self._fc
        value = torch.sigmoid(fn(inputs, hx, output_size, sampled_adj, bias_start=1.0))
        ...
        c = self._gconv(inputs, r * hx, self._num_units, sampled_adj)
    ...
    def _gconv(self, inputs, state, output_size, sampled_adj, bias_start=0.0):
        ...
        self._supports = sampled_adj
        if self._max_diffusion_step == 0:
            pass
        else:
            for support in self._supports:
                # x1 = torch.mm(support.to(x0.device), x0)
                x1 = torch.einsum("bnm, bmd -> bnd", support.to(x0.device), x0)
                x = self._concat(x, x1)

                for k in range(2, self._max_diffusion_step + 1):
                    x2 = 2 * torch.einsum("bnm, bmd -> bnd", support.to(x0.device), x1) - x0
                    x = self._concat(x, x2)
                    x1, x0 = x2, x1
